...
begin
	...
	eNa: $(@bind eNa NumberField(-100:0.1:200, 66.6))\
	gK: $(@bind gK NumberField(-100:0.1:200, 36))\
	eK: $(@bind eK NumberField(-100:0.1:200, -89.1))\
	gCa: $(@bind gCa NumberField(-100:0.1:200, 0.1))\
	eCa: $(@bind eCa NumberField(-100:0.1:200, 130.9))\
	gL: $(@bind gL NumberField(-100:0.01:200, 0.3))\
	eL: $(@bind eL NumberField(-100:0.01:200, -65))\
...
begin
	...
	VD_Na_K_Ca = (t, Y, I) -> [((0.1 * (Y[5] + 40)) / (-exp(-0.1 * (Y[5] + 40)) + 1)) * (1 - Y[1]) - (4.4 * exp(-(Y[5] + 65) / 18)) * Y[1];
		...
		((a1 * (Y[5] + 30)) / (-exp(- 0.1 * (Y[5] + 30)) + 1)) * (1 - Y[4]) - (a2 * exp(-(Y[5] + 80) / a3)) * Y[4];
