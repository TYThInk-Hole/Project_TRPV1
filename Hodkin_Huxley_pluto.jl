...
begin
	...
	eNa: $(@bind eNa NumberField(-100:0.1:200, 70.53))\
	gK: $(@bind gK NumberField(-100:0.1:200, 36))\
	eK: $(@bind eK NumberField(-100:0.1:200, -86.24))\
	gCa: $(@bind gCa NumberField(-100:0.1:200, 0.1))\
	eCa: $(@bind eCa NumberField(-100:0.1:200, 132.34))\
	gL: $(@bind gL NumberField(-100:0.01:200, 0.3))\
	eL: $(@bind eL NumberField(-100:0.01:200, -95.02))\
	start: $(@bind start NumberField(0:1:100, 10))\
	finish: $(@bind finish NumberField(0:1:100, 15))\
	a1: $(@bind a1 NumberField(0:0.01:200, 0.1))\
	a2: $(@bind a2 NumberField(0:0.01:100, 0.01))\
	a3: $(@bind a3 NumberField(0:0.01:100, 20))\
	"""
end
...
VD_Na_K_Ca = (t, Y, I) -> [((0.1 * (Y[5] +50)) / (-exp(-0.1 * (Y[5] + 50)) + 1)) * (1 - Y[1]) - (4.4 * exp(-(Y[5] + 30) / 18)) * Y[1];
		...
		((a1 * (Y[5] + 55)) / (-exp(- 0.1 * (Y[5] + 55)) + 1)) * (1 - Y[4]) - (a2 * exp(-(Y[5] + 30) / a3)) * Y[4];
