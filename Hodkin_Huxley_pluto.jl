...
# ╔═╡ 0e411dfc-0776-11ef-133f-bb32c23513e5
begin
...
# ╔═╡ 3aca2bfa-a629-4b5b-8018-121e65b2e87c
begin
	md"""
	Current: $(@bind param NumberField(-100:0.01:2000, 0))\
	...
	gCa: $(@bind gCa NumberField(-100:0.01:200, 0.1))\
	eCa: $(@bind eCa NumberField(-100:0.1:200, 132.34))\
	gL: $(@bind gL NumberField(-100:0.01:200, 0.3))\
	eL: $(@bind eL NumberField(-100:0.01:200, 10.6))\
	start: $(@bind start NumberField(0:1:100, 10))\
	finish: $(@bind finish NumberField(0:1:100, 15))\
	a1: $(@bind a1 NumberField(0:0.1:200, 65))\
	a2: $(@bind a2 NumberField(0:0.1:100, 65))\
	a3: $(@bind a3 NumberField(0:0.1:100, 65))\
	a4: $(@bind a4 NumberField(0:0.1:100, 65))\
	a5: $(@bind a5 NumberField(0:0.1:100, 65))\
	a6: $(@bind a6 NumberField(0:0.1:100, 65))\
	a7: $(@bind a7 NumberField(0:0.1:100, 65))\
	a8: $(@bind a8 NumberField(0:0.1:100, 65))\
	"""
end

# ╔═╡ 09f0cd5c-55d0-4019-99f2-d5e593c603b4
begin
	tspan = (0,100);
	dt = 0.02;
end

# ╔═╡ ab0edde3-1e59-401c-b594-3ea76055e884
begin
	function runge_kutta4(f, t0, tf, y0, N, param, start, finish)
	    # Initialize arrays
		Currents=ones(1,N+1)*0
		Currents[1,start*Int(N/(tf-t0)):finish*Int(N/(tf-t0))].=param
	    ...
	    return t, dy, y, Currents
	end
end

# ╔═╡ 5d817fb0-d0f7-44bf-a4c3-0e6ec0e7ef12
begin
	function HH_model(MC)
		if MC == "Na_Ca"
			u0 = [0.5, 0.06, 0.01, -65]

			t, v, I = runge_kutta4(VD_Na_Ca,tspan[1],tspan[2],u0,Int(tspan[2]/dt),param,start,finish);
		...
		elseif MC == "Na_K"
			u0 = [0.02, 0.01, 0.01, -56]

			t, dv, v, I =runge_kutta4(VD_Na_K,tspan[1],tspan[2],u0,Int(tspan[2]/dt),param,start,finish);
...
# ╔═╡ 51caa0cd-6114-4c1f-a297-c278d2816864
HH_model("Na_K")

# ╔═╡ 7e9af3e4-9f4b-448f-8a8b-f905fc268a39
begin
	struct MySlider 
	    range::AbstractRange
	    default::Number
...
# ╔═╡ 6c438a64-8638-4276-b84b-2349063d12a1
# ╠═╡ disabled = true
#=╠═╡
begin
VD_Na_K_Ca = (t, Y, I) -> [((0.1 * (Y[5] +50)) / (-exp(-0.1 * (Y[5] + 50)) + 1)) * (1 - Y[1]) - (4.4 * exp(-(Y[5] + 30) / 18)) * Y[1];
		
	            (0.07 * exp(-(Y[5] + 65) / 20)) * (1 - Y[2]) - (1 / (exp(- 0.1 * (Y[5] + 40)) + 1)) * Y[2];
		
	            ((0.01 * (Y[5] + 55)) / (-exp(- 0.1 * (Y[5] + 55)) + 1)) * (1 - Y[3]) - (0.125 * exp(-(Y[5] + 20) / 50)) * Y[3];
		
		((a1 * (Y[5] + a2)) / (-exp(- a3 * (Y[5] + 55)) + 1)) * (1 - Y[4]) - (a4 * exp(-(Y[5] + a5) / a6)) * Y[4];
		
	            (gNa * Y[1]^3 * Y[2] * (eNa - Y[5]) + gK * Y[3]^4 * (eK - Y[5]) + gCa * Y[4] * (eCa - Y[5]) + gL * (eL - Y[5]) + I)]
end
  ╠═╡ =#

# ╔═╡ 8b685685-d343-44c3-9d21-5a9294b8ab04
begin
	VD_Na_Ca = (t, Y, I) -> [((2.5 - 0.1 * (Y[4] + 40)) / (exp(2.5 - 0.1 * (Y[4] + 40)) - 1)) * (1 - Y[1]) - (4.4 * exp(-(Y[4] + 65) / 18)) * Y[1];
	...
	# VD_Na_K = (t, Y, I) -> [((0.1 * (Y[4] + 40)) / (-exp(-0.1 * (Y[4] + 40)) + 1)) * (1 - Y[1]) - (4.4 * exp(-(Y[4] + 65) / 18)) * Y[1];
	#             (0.07 * exp(-(Y[4] + 65) / 20)) * (1 - Y[2]) - (1 / (exp(- 0.1 * (Y[4] + 35)) + 1)) * Y[2];
	#             ((0.01 * (Y[4] + 55)) / (-exp(- 0.1 * (Y[4] + 55)) + 1)) * (1 - Y[3]) - (0.125 * exp(-(Y[4] + 65) / 80)) * Y[3];
	#             (gNa * Y[1]^3 * Y[2] * (eNa - (Y[4] + 65)) + gK * Y[3]^4 * (eK - (Y[4] + 65)) + gL * (eL - (Y[4] + 65)) + I)]
	        ######################################################################################################
	        ######################################################################################################
	    
	# VD_Na_K_Ca = (t, Y, I) -> [((2.5 - 0.1 * (Y[5] + 65.0)) / (exp(2.5 - 0.1 * (Y[5] + 65.0)) - 1)) * (1 - Y[1]) - (4.4 * exp(-(Y[5] + 65.0) / 18)) * Y[1];
	#             (0.6 * exp(-(Y[5] + 65.0) / 20)) * (1 - Y[2]) - (1 / (exp(3.0 - 0.1 * (Y[5] + 65.0)) + 1)) * Y[2];
	#             ((0.1 - 0.01 * (Y[5] + 65.0)) / (exp(1 - 0.1 * (Y[5] + 65.0)) - 1)) * (1 - Y[3]) - (0.125 * exp(-(Y[5] + 65.0) / 80)) * Y[3];
		
	#             # ((1 / (1 + exp(-(Y[5] + 65.0) / 3))) - Y[4]) / (17 * exp((-(Y[5] + 65.0) .^ 2) / 600) + 1.5);
	#             (gNa * Y[1]^3 * Y[2] * (eNa - (Y[5] + 0.0)) + gK * Y[3]^4 * (eK - (Y[5] + 0.0)) + gCa * Y[4] * (eCa - (Y[5] + 0.0)) + gL * (eL - (Y[5] + 0.0)) + I)]

	######################################################################################################
	        ######################################################################################################

	VD_Na_K_Ca = (t, Y, I) -> [((0.1 * (Y[5] + 55)) / (-exp(-0.1 * (Y[5] + 60)) + 1)) * (1 - Y[1]) - (4.4 * exp(-(Y[5] + 55) / 18)) * Y[1];
		
	            (0.07 * exp(-(Y[5] + 55) / 20)) * (1 - Y[2]) - (1 / (exp(- 0.1 * (Y[5] + 55)) + 1)) * Y[2];
		
	            ((0.01 * (Y[5] + 55)) / (-exp(- 0.1 * (Y[5] + 55)) + 1)) * (1 - Y[3]) - (0.125 * exp(-(Y[5] + 55) / 80)) * Y[3];
		
		((a1 * (Y[5] + 55)) / (-exp(- 0.1 * (Y[5] + 55)) + 1)) * (1 - Y[4]) - (a2 * exp(-(Y[5] + 55) / a3)) * Y[4];
...
manifest_format = "2.0"
project_hash = "d72adfcab1d967b4eff15cb8421ea22db8a85e79"
